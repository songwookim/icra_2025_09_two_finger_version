...
def compute_force_variants(df: pd.DataFrame, fs_s2: float, fs_s3: float) -> Tuple[OrderedDict[str, np.ndarray], float]:
    ...
    samples = len(forces_s2)
    rest_samples = max(1, int(0.1 * samples))

    def center_force(force: np.ndarray) -> np.ndarray:
        baseline = np.median(force[:rest_samples, :], axis=0, keepdims=True)
        return force - baseline

    s2_centered = center_force(forces_s2)
    s3_centered = center_force(forces_s3)
...
def main():
    ...
    generated_keys = [key for key in emg_variants_all.keys() if key.startswith('lowpass') and key not in desired_emg]
    ...
    if not args.no_plot:
        fig_emg, axes_emg = plt.subplots(len(emg_variants), 1, figsize=(12, 2.5 * len(emg_variants)), sharex=True)
        if len(emg_variants) == 1:
            axes_emg = [axes_emg]
        for ax, (name, emg_proc) in zip(axes_emg, emg_variants.items()):
            for ch in range(emg_proc.shape[1]):
                ax.plot(time_emg, emg_proc[:, ch], linewidth=0.8, alpha=0.6)
            ax.set_ylabel(name)
            ax.grid(True, alpha=0.3)
        axes_emg[-1].set_xlabel('Time (s)')
        fig_emg.suptitle('EMG Variants (per channel)')
        fig_emg.tight_layout(rect=(0.0, 0.0, 1.0, 0.97))

        # Frequency-domain comparison for key variants (channel 1)
        freq_variants = [name for name in emg_variants if name.startswith('lowpass')]
        ...
        for idx_axis, axis_label in enumerate(axis_labels):
            ...
            if s2_signed is not None:
                axf.plot(time, s2_signed[:, idx_axis], label=f's2_{axis_label} signed', color='#1f77b4', linewidth=1.0)
            if s2_abs is not None:
                axf.plot(time, s2_abs[:, idx_axis], label=f's2_{axis_label} abs', color='#1f77b4', linestyle='--', linewidth=1.2)
            if s3_signed is not None:
                axf.plot(time, s3_signed[:, idx_axis], label=f's3_{axis_label} signed', color='#ff7f0e', linewidth=1.0)
            if s3_abs is not None:
                axf.plot(time, s3_abs[:, idx_axis], label=f's3_{axis_label} abs', color='#ff7f0e', linestyle='--', linewidth=1.2)
            axf.set_ylabel(f'F_{axis_label}')
            axf.grid(True, alpha=0.3)
            axf.legend(loc='upper right', ncol=2)
        axes_force[-1].set_xlabel('Time (s)')
        fig_force.suptitle('Force Components (sensor-wise, signed vs absolute)')
