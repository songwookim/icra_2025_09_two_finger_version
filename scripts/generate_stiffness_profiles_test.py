...
def _compute_centered_force(df: pd.DataFrame) -> np.ndarray:
    force_prefixes = ("force_3", "s3", "force3")
    picked: Optional[List[str]] = None
    for prefix in force_prefixes:
        cand = [f"{prefix}_fx", f"{prefix}_fy", f"{prefix}_fz"]
...
def _extra_smooth_emg(emg_mag: np.ndarray, fs: Optional[float], cutoff_hz: float = 1.0, window_sec: float = 0.6) -> Optional[np.ndarray]:
    if emg_mag is None or emg_mag.size == 0:
        return None
    smoothed = np.asarray(emg_mag, dtype=float)
    if fs is not None and fs > 0 and cutoff_hz > 0 and fs > 2 * cutoff_hz:
...
def _plot_comparison(
    ...
    fig, axes = plt.subplots(3, 1, figsize=(12, 8.0), sharex=True)
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c"]
    for axis in range(min(3, series_a.shape[1])):
        ax = axes[axis]
        ax.plot(time, series_a[:, axis], color=colors[axis % len(colors)], linewidth=1.2, label=label_a)
        ax.plot(time, series_b[:, axis], color=colors[axis % len(colors)], linewidth=1.2, linestyle="--", label=label_b)
        ax.set_ylabel(f"K{axis + 1}")
        ax.grid(alpha=0.3)
...
def _plot_dual_masks(
    ...
    fig, axes = plt.subplots(3, 1, figsize=(12, 8.0), sharex=True)
    colors = ["#1f77b4", "#ff7f0e", "#2ca02c"]
    for axis in range(min(3, positive.shape[1])):
        ax = axes[axis]
        ax.plot(time, positive[:, axis], color=colors[axis % len(colors)], linewidth=1.2, label="force >= 0")
        ax.plot(time, negative[:, axis], color=colors[axis % len(colors)], linewidth=1.2, linestyle="--", label="force < 0")
        ax.set_ylabel(f"K{axis + 1}")
        ax.grid(alpha=0.3)
        ax.legend(loc="upper right", fontsize=8)
    axes[-1].set_xlabel("Time [s]")
...
def process_file(csv_path: Path, output_dir: Path) -> Optional[List[Path]]:
    ...
    try:
        ...
        return None
    try:
        forces_centered = _compute_centered_force(df)
    except Exception as exc:
        print(f"[skip] {csv_path.name}: centered force extraction failed ({exc})")
        return None
    ...
    extra_emg = _extra_smooth_emg(emg_magnitude, fs)

    P_raw = emg_magnitude.T
    F_mat = forces_centered.T
    ...
    # Process ultra_smooth variant
    emg_variant = ultra_emg if ultra_emg is not None else emg_magnitude
    P_variant = emg_variant.T
    stiffness_variant = (T_K @ P_variant).T + K_INIT
    out_png = output_dir / f"{csv_path.stem}_ultra_smooth.png"
    try:
        _plot_variant(
            time,
            forces_signed,
        ...
        print(f"[warn] {csv_path.name}: ultra-smooth plot failed ({exc})")

    # Process extra_smooth variant
    emg_variant_extra = extra_emg if extra_emg is not None else emg_magnitude
    P_variant_extra = emg_variant_extra.T
    stiffness_variant_extra = (T_K @ P_variant_extra).T + K_INIT
    out_png_extra = output_dir / f"{csv_path.stem}_smooth.png"
    try:
        _plot_variant(
            time,
            forces_signed,
            stiffness_raw,
            stiffness_variant_extra,
            emg_magnitude,
            emg_variant_extra,
            out_png_extra,
            csv_path.stem,
        ...
        out_paths.append(out_png_extra)
    except Exception as exc:
        print(f"[warn] {csv_path.name}: extra-smooth plot failed ({exc})")
